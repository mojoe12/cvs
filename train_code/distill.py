...
class MultiLabelImageDataset(Dataset):
    def __init__(self, image_dir, labels_and_confidences, student_height, teacher_height, augment, pad_and_not_crop):
        ...
        self.transform_student = transforms.Compose([
            square_transform,
            transforms.Resize((student_height, student_height)),
            transforms.ToTensor()
        ])
        self.transform_teacher = transforms.Compose([
            square_transform,
            transforms.Resize((teacher_height, teacher_height)),
            transforms.ToTensor()
        ])
    ...
    def __getitem__(self, idx):
        ...
        # Create both versions from the SAME augmented image
        image_student = self.transform_student(image)
        image_teacher = self.transform_teacher(image)

        # Random erase applied only to student
        if self.augment:
            image_student = self.random_erase(image_student)
        ...
        return image_student, image_teacher, label, confidence
...
class MultiLabelVideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        video_images, video_timages, video_labels, video_confidences = [], [], [], []
        for index in self.video_indices[idx]:
            image, timage, label, confidence = self.image_dataset[index]
            video_images.append(image)
            video_timages.append(timage)
            video_labels.append(label)
            video_confidences.append(confidence)
        return torch.stack(video_images), torch.stack(video_timages), torch.stack(video_labels), torch.stack(video_confidences)
...
def parse_args():
    ...
    parser.add_argument('--teacher_model', type=str, required=True, help='Path to Transformer model specification')
    parser.add_argument('--student_model', type=str, required=True, help='Path to Transformer model specification')
...
def main():
    ...
    teacher_model = TransformerMLCModel(num_labels, args.teacher_model).to(device)
    ...
    model = TransformerMLCModel(num_labels, args.student_model).to(device)
